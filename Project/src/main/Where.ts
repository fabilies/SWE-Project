export type FilterBuilder<T> = {
    entity: T,
    get: <key extends keyof T>(key: key) => WhereOperator<T, T[key]> // Get key and append operator to execute
}
...
export const FilterBuilder = <T>(entity: T): FilterBuilder<T> => ({
    entity: entity,
    get: function <key extends keyof T>(key: key): WhereOperator<T, T[key]> {
...
let FilterCondition = <T>(e: T, cond: boolean): FilterCondition<T> => ({
    entity: e,
    condition: cond,
